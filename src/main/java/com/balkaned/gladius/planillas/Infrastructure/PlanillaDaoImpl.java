package com.balkaned.gladius.planillas.Infrastructure;

import com.balkaned.gladius.banco.Domain.BancoResumenPer;
    ...
    public void procesarPla2020(List<PlaProPeriodo> Persona, Integer codcia, Integer idproceso, String idPeriodo,
        ...
        while (l_formula.hasNext()) {
            ...
            if (for_det.getTipOut().equals("1") || for_det.getTipOut().equals("3")) {
                ...
                // OPTIMIZACIÓN: Crear Map para evitar iteraciones O(N²)
                // Agrupar datos por trabajador para acceso O(1)
                java.util.Map<Integer, StringBuilder> dataByTrabajador = new java.util.HashMap<>();
                for (ProPeriodoDet item : LoadData2) {
                    dataByTrabajador.computeIfAbsent(item.getIexcodtra(), k -> new StringBuilder())
                        .append(" ").append(item.getVarcon()).append("=").append(item.getValue()).append("; ");
                }

                // OPTIMIZACIÓN: Preparar batch updates en lugar de updates individuales
                ...
                while (pi.hasNext()) {
                    ...
                    // OPTIMIZACIÓN: Acceso directo O(1) en lugar de buscar O(N)
                    v_variables_concat = dataByTrabajador.getOrDefault(pi_persona.getIexcodtra(), new StringBuilder()).toString();
                    ...
                    // OPTIMIZACIÓN: Acumular en batch en lugar de ejecutar inmediatamente
                    batchArgs.add(new Object[]{
                        v_resultadoFinal,
                        codcia,
                        idproceso,
                        idPeriodo,
                        pi_persona.getIexcodtra(),
                        for_det.getIdConcepto(),
                        correl
                    });
                }

                // OPTIMIZACIÓN: Ejecutar batch update de una sola vez
                if (!batchArgs.isEmpty()) {
                    String sqlBatchUpdate = "update iexpropertra_nomina_proc " +
                            "set provalor = ? " +
                            "where iexcodcia = ? and " +
                            "iexcodpro = ? and " +
                            "iexnroper = ? and " +
                            ...
                            "iexcorrel = ? ";
                    jdbc.batchUpdate(sqlBatchUpdate, batchArgs);
                }
