...
package com.balkaned.gladius.planillas.Application;
...
import com.balkaned.gladius.empleado.Domain.EmpDatvar;
import com.balkaned.gladius.empleado.Domain.EmpleadoService;
...
import com.balkaned.gladius.planillas.Domain.PlanillaService;
import com.balkaned.gladius.services.*;
import com.balkaned.gladius.servicesImpl.Sessionattributes;
import com.balkaned.gladius.models.WorkerThread;
import com.balkaned.gladius.sueldos.Domain.SueldoService;
...
public class PlanillaController {
    ...
    public ModelAndView procesarPlanilla(ModelMap model, HttpServletRequest request) {
        ...
        if (accion.equals("EXEPRO")) {
            ...
            while (pi.hasNext()) {
                ...
                // Solo loguear cada 50 trabajadores para reducir I/O
                if (counter % 50 == 0) {
                    log.debug("Distribuyendo trabajadores... Procesados: " + counter);
                }
                counter++;
            }

            log.info("Distribución de trabajadores completada. Total: " + counter + " trabajadores");
            ...
            log.debug("Total empleados en lp_persona_s1: " + lp_persona_s1.size());
            log.debug("Total empleados en lp_persona_s2: " + lp_persona_s2.size());
            log.debug("Total empleados en lp_persona_s3: " + lp_persona_s3.size());
            log.debug("Total empleados en lp_persona_s4: " + lp_persona_s4.size());

            // Crear tareas Runnable para cada grupo de empleados
            Runnable runnable1 = new Runnable() {
                @Override
                public void run() {
                    log.info(Thread.currentThread().getName() + " Start. Procesando grupo 1 con " + lp_persona_s1.size() + " empleados");
                    try {
                        planillaDao.procesarPla2020(lp_persona_s1, idCompania, iexcodpro, iexperiodo, iexcodtra, iexcorrel, 1);
                        log.info(Thread.currentThread().getName() + " End. Grupo 1 completado exitosamente");
                    } catch (Exception e) {
                        log.error("Error en Hilo1: " + e.getMessage(), e);
                        throw new RuntimeException("Error procesando grupo 1", e);
                    }
                }
            };
            ...
            Runnable runnable2 = new Runnable() {
                @Override
                public void run() {
                    log.info(Thread.currentThread().getName() + " Start. Procesando grupo 2 con " + lp_persona_s2.size() + " empleados");
                    try {
                        planillaDao.procesarPla2020(lp_persona_s2, idCompania, iexcodpro, iexperiodo, iexcodtra, iexcorrel, 2);
                        log.info(Thread.currentThread().getName() + " End. Grupo 2 completado exitosamente");
                    } catch (Exception e) {
                        log.error("Error en Hilo2: " + e.getMessage(), e);
                        throw new RuntimeException("Error procesando grupo 2", e);
                    }
                }
            };
            ...
            Runnable runnable3 = new Runnable() {
                @Override
                public void run() {
                    log.info(Thread.currentThread().getName() + " Start. Procesando grupo 3 con " + lp_persona_s3.size() + " empleados");
                    try {
                        planillaDao.procesarPla2020(lp_persona_s3, idCompania, iexcodpro, iexperiodo, iexcodtra, iexcorrel, 3);
                        log.info(Thread.currentThread().getName() + " End. Grupo 3 completado exitosamente");
                    } catch (Exception e) {
                        log.error("Error en Hilo3: " + e.getMessage(), e);
                        throw new RuntimeException("Error procesando grupo 3", e);
                    }
                }
            };
            ...
            Runnable runnable4 = new Runnable() {
                @Override
                public void run() {
                    log.info(Thread.currentThread().getName() + " Start. Procesando grupo 4 con " + lp_persona_s4.size() + " empleados");
                    try {
                        planillaDao.procesarPla2020(lp_persona_s4, idCompania, iexcodpro, iexperiodo, iexcodtra, iexcorrel, 4);
                        log.info(Thread.currentThread().getName() + " End. Grupo 4 completado exitosamente");
                    } catch (Exception e) {
                        log.error("Error en Hilo4: " + e.getMessage(), e);
                        throw new RuntimeException("Error procesando grupo 4", e);
                    }
                }
            };

            // Usar ExecutorService para gestionar los hilos de manera segura
            ExecutorService executor = Executors.newFixedThreadPool(4);
            List<Exception> exceptions = new ArrayList<>();

            try {
                log.info("Iniciando procesamiento paralelo con 4 hilos...");

                // Enviar tareas al executor y obtener Futures para verificar resultados
                java.util.concurrent.Future<?> future1 = executor.submit(runnable1);
                java.util.concurrent.Future<?> future2 = executor.submit(runnable2);
                java.util.concurrent.Future<?> future3 = executor.submit(runnable3);
                java.util.concurrent.Future<?> future4 = executor.submit(runnable4);

                // Esperar a que todos los hilos terminen y capturar excepciones
                try {
                    future1.get();
                } catch (Exception e) {
                    log.error("Excepción en future1: " + e.getMessage(), e);
                    exceptions.add(e);
                }
                ...
                try {
                    ...
                    log.error("Excepción en future2: " + e.getMessage(), e);
                    exceptions.add(e);
                }
                ...
                try {
                    ...
                    log.error("Excepción en future3: " + e.getMessage(), e);
                    exceptions.add(e);
                }
                ...
                try {
                    ...
                    log.error("Excepción en future4: " + e.getMessage(), e);
                    exceptions.add(e);
                }

                log.info("Todos los hilos han finalizado su ejecución");

            } catch (Exception e) {
                log.error("Error general en el procesamiento paralelo: " + e.getMessage(), e);
                exceptions.add(e);
            } finally {
                // Apagar el executor de manera ordenada
                executor.shutdown();
                try {
                    // Esperar hasta 1 hora para que terminen las tareas pendientes
                    if (!executor.awaitTermination(60, java.util.concurrent.TimeUnit.MINUTES)) {
                        log.warn("El executor no terminó en el tiempo esperado, forzando apagado...");
                        executor.shutdownNow();
                    }
                } catch (InterruptedException e) {
                    log.error("Interrupción durante el apagado del executor", e);
                    executor.shutdownNow();
                    Thread.currentThread().interrupt();
                }
            }

            // Si hubo excepciones en algún hilo, registrarlas
            if (!exceptions.isEmpty()) {
                log.error("Se encontraron " + exceptions.size() + " errores durante el procesamiento paralelo");
                for (int i = 0; i < exceptions.size(); i++) {
                    log.error("Error " + (i + 1) + ": " + exceptions.get(i).getMessage());
                }
                // IMPORTANTE: No lanzamos excepción aquí para que continúe el proceso
                // pero queda registrado en los logs
            }

            log.info("Procesamiento paralelo finalizado. Procediendo a guardar nómina...");
    ...
    public ModelAndView actualizarMigValorTrabConcept(ModelMap model, HttpServletRequest request,
        ...
        return new ModelAndView("redirect:/migracionPlanilla@" + iexcodreg + "@" + codproceso + "@" + periodo);
    }
    ...
    @RequestMapping(value = "/updateDatosTurnoDia", method = {RequestMethod.POST, RequestMethod.GET})
    public ModelAndView updateDatosTurnoDia(HttpServletRequest request, HttpServletResponse response) throws IOException {
        log.info("/updateDatosTurnoDia");
